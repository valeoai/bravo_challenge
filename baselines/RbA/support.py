...
def get_datasets(datasets_folder):
    ...
    ['ACDC', 'SMIYC', 'outofcontext', 'synflare', 'synobjs', 'synrain']
    ...
    # DATASETS = edict(
    #     road_anomaly_21=RoadAnomaly21(hparams=road_anomaly_21_config, transforms=transform),
    # )
    DATASETS = edict(
        ...
        bravo_synrain=BRAVO(hparams=bravo_synrain_config, transforms=transform)
    )
...
class OODEvaluator:
    ...
    def compute_anomaly_scores(
        ...
        for x, y, filepath in tqdm(loader, desc="Dataset Iteration"):
            if jj >= upper_limit:
                break
            jj += 1
            ...
            # save the prediction
            mmcv.imwrite(savepred, destfile_pred)
            # mmcv.imwrite(conf, destfile_conf)
            # save the conf as lossless webp to save space
